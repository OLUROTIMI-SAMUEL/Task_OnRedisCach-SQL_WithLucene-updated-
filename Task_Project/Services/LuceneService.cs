...
namespace Task_Project.Services
{
    public class LuceneService
    {
        ...
        public LuceneService(CustomerDb dbContext)
        {
        ...
        {
            // Check if the Lucene index directory contains any files
        ...
        public void IndexCustomers()
        {
            ...
            using (IndexWriter writer = new IndexWriter(directory, config))
            {
                using (IndexReader reader = DirectoryReader.Open(writer, true))
                {
                    ...
                    foreach (var customer in _dbContext.CustomersDetails)
                    {
        ...
        public List<Customers> SearchById(int id)
        {
        ...
        public List<Customers> SearchByAddress(string address)
        {
            return Search("Address", address);
        }

        public List<Customers> SearchByEmail(string email)
        {
            return Search("Email_Address", email, useKeywordAnalyzer: true);
        }

        private List<Customers> Search(string field, string searchTerm, bool useKeywordAnalyzer = false)
        {
            FSDirectory directory = FSDirectory.Open(new DirectoryInfo(IndexPath));
        ...
        private void IndexCustomer(IndexWriter writer, Customers customer)
        {
